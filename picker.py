...
def __read_and_sort(fileobj, delimiter=","):
    def key(pair):
        word, _ = pair
        return len(word)
    vocabs = sorted(csv.reader(fileobj, delimiter=delimiter), key=key, reverse=True)
...
def pick_puzzles(fileobj, grid_size_leq, stop_picking_at, random_seed, print_interval, delimiter=","):
	random.seed(random_seed)
	puzzles = []
	sorted_words, sorted_descs = __read_and_sort(fileobj, delimiter=delimiter)
